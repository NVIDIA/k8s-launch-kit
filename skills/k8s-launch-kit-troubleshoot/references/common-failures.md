...
**Remediation**:
1. Check `ofedDependentModules` in your cluster config -- these modules must be blacklisted.
2. Set `docaDriver.blacklistDependentModules: true` in your l8k config.
3. Re-run discovery (`--discover-cluster-config`) to refresh the dependent modules list.
4. Redeploy. The generated NicClusterPolicy will include `OFED_BLACKLIST_MODULES` env var
