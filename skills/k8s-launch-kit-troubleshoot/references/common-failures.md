...
**Remediation**:
1. Check `ofedDependentModules` in your cluster config -- these modules must be unloaded.
2. Set `docaDriver.unloadDependentModules: true` in your l8k config.
3. Re-run discovery (`--discover-cluster-config`) to refresh the dependent modules list.
4. Redeploy. The generated NicClusterPolicy will include `UNLOAD_CUSTOM_MODULES` env var
