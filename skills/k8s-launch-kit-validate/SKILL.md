...
Validate constructs one strict desired-artifact snapshot before cluster
checks. Malformed or nameless documents fail with source details and exit 2;
`values.yml` and case variants must be renamed to `values.yaml`. `--wait`,
connectivity, and reports reuse that desired snapshot while reading fresh
cluster state.
