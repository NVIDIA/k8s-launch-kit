...
The `docaDriver` section controls the OFED driver deployment in the generated NicClusterPolicy or NicNodePolicy. Set `enable: true` to include the `ofedDriver` section in generated manifests, or `enable: false` to omit it. This can also be overridden via the `--enable-doca-driver` CLI flag.
...
Advanced users can pass literal environment variables to both MOFED
containers through `docaDriver.env`. Custom entries override variables
generated by the first-class `docaDriver` fields when the names match, and the
rendered policy contains only one entry per name. When a custom name is listed
...
Use this escape hatch only for driver settings that Launch Kit does not model.
Incorrect values can prevent the driver from reloading and disrupt node
networking.
