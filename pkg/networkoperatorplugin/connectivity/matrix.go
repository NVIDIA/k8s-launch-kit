...
// test bucket. Three test families are scheduled in stages by the
// orchestrator:
//
//   - Kind = PingSameRail / PingCrossRail — IPv4 `ping -c N`. Always
//     runs first; subsequent RDMA stages only schedule pairs that
//     passed ICMP on the same rail.
...
// SrcRDMADev / DstRDMADev are populated for RDMA tests so the test
// runner can pass `-d <dev>`; empty for ICMP tests.
type PingTest struct {
	...
	SrcRail    string
	DstRail    string
	SrcRDMADev string // populated for RDMA test kinds; empty for ICMP
	DstRDMADev string
}

// PingTestKind enumerates the buckets the matrix renders into. Order
// matters for the staged-execution flow: ICMP first, RDMA ping second,
// ib_write_bw last (each subsequent stage gates on the previous).
...
// IsICMP reports whether the test kind is an IPv4 ping (vs an RDMA
// test). Used by the renderer to group tests under the right
// per-rail sub-header.
func (k PingTestKind) IsICMP() bool {
	return k == PingSameRail || k == PingCrossRail
}
...
// canary variants (regardless of ICMP / RDMA family).
func (k PingTestKind) IsCrossRail() bool {
	return k == PingCrossRail || k == RDMAPingCrossRail || k == RDMABwCrossRail
}
...
// deterministic regardless of the cluster's pod-listing order. The
// per-Kind slices are populated lazily — same-rail / cross-rail
// always; RDMA variants only when RDMAPlan() is called against an
// existing plan (so the orchestrator can stage ICMP first, then
// build the RDMA stage from the pairs that survived).
...
// RDMAPlan extends an existing ICMP plan with rping and ib_write_bw
// test variants. Called after the ICMP stage completes — the
// orchestrator passes in the same TestPod list, and we mint
// per-pair-per-rail tests using each pod's RDMADevsByRail map.
//
...
//
// rping and ib_write_bw are emitted as separate Kind variants so the
// renderer can lay out one matrix per family. The "both directions"
// shape (per the user decision) means every ordered pod pair gets
// its own server/client pairing.
func RDMAPlan(pods []TestPod, plan *MatrixPlan) {
	if plan == nil || plan.Skip != nil {
		return
	}
	sorted := make([]TestPod, len(pods))
	copy(sorted, pods)
	...
	for i, src := range sorted {
		for j, dst := range sorted {
			...
			// Cross-rail canary: same pair, rail[0] → rail[1].
			if len(src.RailOrder) >= 2 && len(dst.RailOrder) >= 2 {
				srcRail, dstRail := src.RailOrder[0], dst.RailOrder[1]
				srcIP, ok1 := src.IPsByRail[srcRail]
				dstIP, ok2 := dst.IPsByRail[dstRail]
