...
func TestBuildProfileFromLLMResponse(t *testing.T) {
	p := &NetworkOperatorPlugin{}

	t.Run("basic non-spectrum-x response", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":         "ethernet",
			"deploymentType": "sriov",
			"multirail":      "true",
			"spectrumX":      "false",
	...
	t.Run("spectrum-x with all fields provided CX8 hwplb", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":                  "ethernet",
			"deploymentType":          "sriov",
			"multirail":               "true",
	...
	t.Run("spectrum-x with missing sub-fields falls back to defaults", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":         "ethernet",
			"deploymentType": "sriov",
			"multirail":      "true",
	...
	t.Run("BF3 with multiplane none forces 1 plane", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":                  "ethernet",
			"deploymentType":          "sriov",
			"multirail":               "true",
		...
		require.NoError(t, err)
		require.NotNil(t, profile.SpectrumX)
		assert.Equal(t, "none", profile.SpectrumX.MultiplaneMode)
		assert.Equal(t, 1, profile.SpectrumX.NumberOfPlanes)
	})

	t.Run("uniplane forces 1 plane", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":                  "ethernet",
			"deploymentType":          "sriov",
			"multirail":               "true",
		...
		require.NoError(t, err)
		require.NotNil(t, profile.SpectrumX)
		assert.Equal(t, "uniplane", profile.SpectrumX.MultiplaneMode)
		assert.Equal(t, 1, profile.SpectrumX.NumberOfPlanes)
	})

	t.Run("swplb with 4 planes", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":                  "ethernet",
			"deploymentType":          "sriov",
			"multirail":               "true",
	...
	t.Run("spectrum-x enforces ethernet fabric and sriov", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":         "",
			"deploymentType": "",
			"multirail":      "false",
		...
		assert.Equal(t, "ethernet", profile.Fabric)
		assert.Equal(t, "sriov", profile.Deployment)
		assert.True(t, profile.Multirail) // forced by spectrumX
	})

	t.Run("invalid numberOfPlanes falls back to default", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":                  "ethernet",
			"deploymentType":          "sriov",
			"multirail":               "true",
		...
		require.NoError(t, err)
		require.NotNil(t, profile.SpectrumX)
		assert.Equal(t, 4, profile.SpectrumX.NumberOfPlanes) // fallback
	})

	t.Run("infiniband non-spectrum-x response", func(t *testing.T) {
		llmResponse := map[string]string{
			"fabric":         "infiniband",
			"deploymentType": "sriov",
			"multirail":      "false",
		...
		err := p.BuildProfileFromLLMResponse(llmResponse, profile)
		require.NoError(t, err)
		assert.Equal(t, "infiniband", profile.Fabric)
		assert.Equal(t, "sriov", profile.Deployment)
		assert.False(t, profile.Multirail)
		assert.False(t, profile.Ai)
		assert.Nil(t, profile.SpectrumX)
