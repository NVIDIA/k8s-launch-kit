...
func TestDefaultLaunchKitConfig(t *testing.T) {
	...
	assert.Equal(t, "26.7", cfg.NetworkOperator.SelectedRelease)
	assert.Equal(t, "v26.7.0", cfg.NetworkOperator.Version)
	assert.Equal(t, "network-operator-v26.7.0", cfg.NetworkOperator.ComponentVersion)
...
func TestDOCADriverEnvConfig(t *testing.T) {
	t.Run("loads literal environment values", func(t *testing.T) {
		var cfg LaunchKitConfig
		err := yaml.Unmarshal([]byte(`docaDriver:
  env:
...
`), &cfg)
		...
		require.Equal(t, []DOCADriverEnvVar{{
			Name:  "THIRD_PARTY_RDMA_MODULES",
			Value: "nvidia_peermem",
		}}, cfg.DOCADriver.Env)
	})

	t.Run("rejects invalid environment names", func(t *testing.T) {
		err := validateDOCADriverConfig(&DOCADriverConfig{Env: []DOCADriverEnvVar{{
			Name:  "NOT VALID",
			Value: "value",
		}}})
		require.ErrorContains(t, err, `docaDriver.env[0].name "NOT VALID" is invalid`)
	})

	t.Run("generated cluster config keeps the advanced field commented out", func(t *testing.T) {
		cfg, err := DefaultLaunchKitConfig()
		require.NoError(t, err)
		cfg.ClusterConfig = []ClusterConfig{{Identifier: "discovered"}}

		out, err := MarshalConfigWithComments(
			cfg, DefaultConfigYAML(), "Generated by `l8k discover`.")
		require.NoError(t, err)
		contents := string(out)
		require.Contains(t, contents, "# Advanced users only")
		require.Contains(t, contents, "# env:")
		require.NotContains(t, contents, "\n  env:\n", "the advanced field must remain opt-in")
	})
}
